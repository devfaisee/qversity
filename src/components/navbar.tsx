...
import { FaShoppingCart, FaSearch } from 'react-icons/fa'; // Font Awesome icons import Image from 'next/image'; // Next.js Image component import { useState } from 'react'; import logo from '../../public/qversity_logo.png'; // Import logo image import Link from 'next/link';

const Navbar = () => { const [searchQuery, setSearchQuery] = useState('');

const handleSearch = (e: React.FormEvent) => { e.preventDefault(); if (searchQuery.trim()) { console.log('Searching for:', searchQuery); // Handle search logic or redirect } };
...
return (
   ...
   <nav className="fixed top-0 left-0 w-full bg-white/30 backdrop-blur-md shadow-xl p-4 rounded-b-lg border-b border-white/50 z-50 h-auto lg:h-20 text-gray-900 animate-fade-in">
      ...
      </div>{/* Mobile Navigation Links (Visible in Small Screens) */}
